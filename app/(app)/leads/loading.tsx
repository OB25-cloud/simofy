export default function Loading() {
  return (
    <div className="p-4 md:p-8">
      {/* Header */}
      <div className="mb-6 flex items-center justify-between">
        <div>
          <div className="h-7 w-16 bg-line rounded animate-pulse mb-2" />
          <div className="h-4 w-24 bg-line rounded animate-pulse" />
        </div>
        <div className="h-9 w-24 bg-line rounded animate-pulse" />
      </div>

      {/* 4 stat cards */}
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white rounded-xl border border-line shadow-sm p-4">
            <div className="h-3 w-20 bg-line rounded animate-pulse mb-3" />
            <div className="h-7 w-12 bg-line rounded animate-pulse" />
          </div>
        ))}
      </div>

      {/* Filter bar */}
      <div className="flex gap-3 mb-5">
        <div className="h-10 flex-1 bg-line rounded-lg animate-pulse" />
        <div className="h-10 w-44 bg-line rounded-lg animate-pulse" />
      </div>

      {/* Table */}
      <div className="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div className="h-10 bg-surface-muted border-b border-line" />
        {[...Array(6)].map((_, i) => (
          <div key={i} className="h-14 border-b border-line-soft px-5 flex items-center gap-5">
            <div className="h-3.5 w-28 bg-line rounded animate-pulse" />
            <div className="h-3.5 w-36 bg-line rounded animate-pulse" />
            <div className="h-3.5 w-24 bg-line rounded animate-pulse" />
            <div className="h-3.5 w-40 bg-line rounded animate-pulse" />
            <div className="h-3.5 w-16 bg-line rounded animate-pulse" />
            <div className="h-3.5 w-20 bg-line rounded animate-pulse" />
            <div className="h-5 w-18 bg-line rounded-full animate-pulse ml-auto" />
          </div>
        ))}
      </div>
    </div>
  )
}
