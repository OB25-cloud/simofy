    ...
    <div className="p-4 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-5 flex items-center gap-2 text-sm text-ink-muted">
          <Link href="/leads" className="hover:text-ink-muted transition-colors">Leads</Link>
          <span>/</span>
          <span className="text-ink-muted font-medium">{typedLead.name ?? 'Lead'}</span>
        </div>
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 flex-wrap mb-1">
              <h1 className="text-[26px] leading-tight font-bold tracking-tight text-ink">{typedLead.name ?? 'Unnamed Lead'}</h1>
              <StatusBadge status={typedLead.status} />
            </div>
            {typedLead.source && (
              <p className="text-sm text-ink-muted">via {typedLead.source}</p>
            )}
          </div>
