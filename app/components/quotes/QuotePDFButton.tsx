...
export default function QuotePDFButton({ quote, lineItems }: Props) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      doc.text('Thank you for choosing Runsite — Green & Co Landscaping.', M, H - 9.5)
      doc.setTextColor(...GOLD)
      doc.text('info@runsite.co.nz  ·  runsite.co.nz', W - M, H - 15, { align: 'right' })
  ...
  return (
    <button
      ...
      disabled={busy}
      className="flex items-center gap-1.5 px-4 py-3 sm:py-2 text-sm font-medium rounded-lg bg-surface border border-line text-ink hover:bg-surface-muted transition-colors disabled:opacity-60"
    >
