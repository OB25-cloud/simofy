    ...
    >
      <div className="bg-white w-full h-full sm:h-[92vh] sm:max-w-2xl sm:rounded-xl shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-line shrink-0">
          <h2 className="text-sm font-semibold text-ink">Edit Quote</h2>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              {loadingItems ? (
                <p className="text-xs text-ink-muted py-3 text-center">Loading items…</p>
              ) : (
                  ...
                  {lineItems.map((item, idx) => (
                    <div key={item.id} className="space-y-1.5">
                      ...
                      <div className="grid grid-cols-[60px_1fr_80px_28px] gap-2 items-center">
                        ...
                        <span className="text-sm text-right text-ink-muted tabular-nums">${calc.amounts[idx].toFixed(2)}</span>
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-line shrink-0">
            ...
            <button type="submit" disabled={loading || loadingItems} className="px-4 py-3 sm:py-2 text-sm font-medium text-white font-semibold rounded-md transition-[filter] hover:brightness-110 disabled:opacity-60" style={{ background: 'var(--accent)' }}>
